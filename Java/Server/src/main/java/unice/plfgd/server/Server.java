...
public class Server {
	...
	public Server(Configuration config, Dispatcher requestDispatcher) {
		...
		this.server.addEventListener("message", byte[].class, this::handleIncomingMessage);
	}
	...
	private void handleIncomingMessage(SocketIOClient client, byte[] stream, AckRequest ackRequest) {
		// Did the client send us an error? If yes > we don't answer back and stop processing
		Exchange packet = (Exchange) Serializer.BytesToObject(stream);
		assert packet != null;
