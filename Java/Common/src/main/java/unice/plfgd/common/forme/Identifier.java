...
public class Identifier {
	...
	static public List<Point> sanitize(List<Point> ptList, int tolerance){
		...
		if(maxDistance <= tolerance){
			...
			ArrayList<Point> left = new ArrayList<Point>(sanitize(ptList.subList(0, indexOfFurthestPoint), tolerance));
			ArrayList<Point> right = new ArrayList<Point>(sanitize(ptList.subList(indexOfFurthestPoint, ptList.size()), tolerance));
			if(right.size() != 0) {
				right.remove(0);
			}
